...
void EPT0_CONFIG(void)
{
	...
	EPT_IO_SET(EPT_IO_EPI,IO_NUM_PA07);
	EPT_PWM_Config(EPT_Selecte_PCLK,EPT_CNTMD_decrease,EPT_OPM_Continue,0);//PCLK作为TCLK时钟，递增模式，连续模式，TCLK=PCLK/(0+1)
	...
	EPT_PWMX_Output_Control(EPT_PWMD,EPT_CA_Selecte_CMPD,EPT_CB_Selecte_CMPD,EPT_PWM_ZRO_Event_OutHigh,EPT_PWM_PRD_Event_Nochange,EPT_PWM_CAU_Event_OutLow,EPT_PWM_CAD_Event_OutLow,
							EPT_PWM_CBU_Event_Nochange,EPT_PWM_CBD_Event_Nochange,EPT_PWM_T1U_Event_Nochange,EPT_PWM_T1D_Event_Nochange,EPT_PWM_T2U_Event_Nochange,EPT_PWM_T2D_Event_Nochange);		
	EPT_PRDR_CMPA_CMPB_CMPC_CMPD_Config(1000,500,600,400,200);//PRDR=1000,CMPA=800,CMPB=600,CMPC=400,CMPD=200
	EPT_Tevent_Selecte(0x00,0x00);
	//EPT_EPX_Config(EPT_EP0,EPT_Input_selecte_EPI0,EPT_FLT_PACE0_DIS,EPT_FLT_PACE1_DIS,0X01);		// EPI 低事件有效
	//EPT_SYNCR_Config(EPT_Triggle_Continue,EPT_SYNCUSR0_REARMTrig_DIS,EPT_TRGSRC0_ExtSync_SYNCUSR0,EPT_TRGSRC1_ExtSync_SYNCUSR4,0x10);//使能SYNCUSR4 ,连续触发
	EPT_DB_CLK_Config(0,0,0);//Fdbclk=Fhclk/(0+1)，DTR=10clk，DTF=10clk
	EPT_DBCR_Config(EPT_CHA_Selecte,EPT_CHA_PWMA_RISE_FALL,EPT_CHA_OUTSEL_EnRise_EnFall,EPT_PB_OUT_Reverse,EPT_PAtoCHX_PBtoCHY);//PWMA作为互补输入源,CHX上升沿，CHY下降沿		
	EPT_DBCR_Config(EPT_CHB_Selecte,EPT_CHB_PWMB_RISE_FALL,EPT_CHB_OUTSEL_EnRise_EnFall,EPT_PB_OUT_Reverse,EPT_PAtoCHX_PBtoCHY);//PWMB作为互补输入源,CHX上升沿，CHY下降沿		
	EPT_DBCR_Config(EPT_CHC_Selecte,EPT_CHC_PWMC_RISE_FALL,EPT_CHC_OUTSEL_EnRise_EnFall,EPT_PB_OUT_Reverse,EPT_PAtoCHX_PBtoCHY);//PWMC作为互补输入源,CHX上升沿，CHY下降沿
	//EPT_CPCR_Config(EPT_CPCR_ENALBE,EPT_CPCR_Source_TCLK,EPT_CDUTY_1_8,3,0);//斩波输出使能，TCLK作为Tcpcr时钟源，起始斩波时间3 clk，斩波占空比为1/8
	EPT_TRGSRCX_Config(EPT_TRGSRC0,EPT_EVTRG_TRGSRCX_PRD,EPT_TRGSRC0_DIS,1);//PRD事件作为TRGSRC0触发源,禁止触发输出,触发周期设置为1
	EPT_Global_Eventload_Config(EPT_GLDMD_Selecte_ZRO_PRD,0x3fff);//设置ZRO和PRD为全局载入触发事件，
	
	EPT_Int_Enable(EPT_TRGEV0_INT);//TRGEV0作为中断
	...
	EPT_Int_Enable(EPT_CBP_LD1);//CMPB载入中断
	EPT_Vector_Int_Enable();
	EPT_Start();*/
}
/*************************************************************/
...
void BT_CONFIG(void)
{
	...
	//BT_Period_CMP_Write(BT0, 1000, 1);//定时1ms
	BT_Period_CMP_Write(BT0, 125, 1);//定时0.125ms
	BT_Start(BT0);
...
void SIO_CONFIG(void)
{
	...
	//SIO_TXBUF_Set(TX_D1,TX_D0,TX_D1,TX_D0,TX_DL,TX_DH,TX_DL,TX_DL,TX_DH,TX_D0,TX_D1,TX_D0,TX_D1,TX_D0,TX_D1,TX_D0);
...
void UART0_CONFIG(void)
{
    ...
    UARTInitRxTxIntEn(UART0,SYSCLOCK*1000000/115200,UART_PAR_NONE);	                  //baudrate=sysclock/46=115200//SYSCLOCK*1000000/115200
	UART0_Int_Enable();
}	
/*************************************************************/
//UART1  CONFIG
...
void UART1_CONFIG(void)
{
    ...
    UARTInitRxTxIntEn(UART1,SYSCLOCK*1000000/115200,UART_PAR_NONE);	                    //baudrate=sysclock/46=115200
	UART1_Int_Enable();
}	
/*************************************************************/
//UART2  CONFIG
...
void UART2_CONFIG(void)
{
    ...
    UART_IO_Init(IO_UART2, 0);                                                        	//use PA0.0->RXD2, PA0.1->TXD2
    UARTInitRxTxIntEn(UART2,SYSCLOCK*1000000/115200,UART_PAR_NONE);	                    //baudrate=sysclock/46=115200
	UART2_Int_Enable();
}	
/*************************************************************/
//ifc config
